...
function setupHeroLiquid() {
  ...
  const drawFrame = (now) => {
    ...
    ctx.beginPath();
    const currentRadius = W < 768 ? 160 : radius; 
    ...
    cards.forEach((card, index) => {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          const diff = p - 0.5; 
          const rot = diff * -35; 
          const tx = (1 - Math.cos(diff * Math.PI)) * -20; 
          let opacity = 1;
          if (p < 0.20) {
            opacity = p / 0.20; 
          } else if (p > 0.80) {
  ...
  PRODUCTS_PREVIEW.forEach(p => {
    ...
    card.innerHTML = `
      <div class="cs-prod-img-wrap">
        <img src="${p.image}" alt="${p.name}" class="cs-prod-img" width="300" height="240" loading="lazy" decoding="async">
...
function initScrambled3DCardPile() {
  ...
  cards.forEach(card => {
    ...
    card.addEventListener("mousemove", (e) => {
      ...
      targetScale = 1.12; 
      targetTranslateY = defaults.y - 25; 
      startCardLoop();
    });
