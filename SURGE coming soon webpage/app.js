...
function renderProductsProgressive() {
  ...
  // Trigger cell-division mutation entrance animation once the grid scrolls into view
  initProductGridMutation(grid);
}

// ==========================================
// 5b. Cell-Division Mutation Grid Entrance Animation
// ==========================================
function initProductGridMutation(grid) {
  if (window.innerWidth < 768) {
    // Disable cell mutation entrance animation on mobile
    const cards = Array.from(grid.querySelectorAll(".cs-prod-card"));
    cards.forEach(card => card.style.opacity = "1"); // Ensure all cards are fully visible immediately
    return;
  }

  // Create overlay container for cell animation layer
  const overlay = document.createElement("div");
  overlay.className = "mutation-overlay";
  grid.appendChild(overlay);

  // ScrollTrigger to start the cell division once the grid comes into view
  ScrollTrigger.create({
    trigger: grid,
    start: "top 78%",
    onEnter: () => {
      ...
      // Calculate dynamic layout coordinate centers and dimensions for the 7 product cards
      ...
      // Helper function to spawn a card-sized square cell matching the exact card dimensions
      function createCell(targetIdx) {
        const target = targets[targetIdx];
        const cell = document.createElement("div");
        cell.className = "mutation-cell";
        
        // Dynamically size it to match the product card exactly
        cell.style.width = `${target.w}px`;
        cell.style.height = `${target.h}px`;
      ...
      // ----------------------------------------------------
      // Stage 1: Spawn 1 card-sized square in Row 1 slot 2 (targets[1])
      // ----------------------------------------------------
      const cell2 = createCell(1);
      gsap.set(cell2, { left: targets[1].x, top: targets[1].y });
      gsap.fromTo(cell2, 
        { scale: 0.8, opacity: 0 }, 
      ...
      // ----------------------------------------------------
      // Stage 2: 1 -> 4 Squares (splits horizontally across Row 1)
      // ----------------------------------------------------
      let cell1, cell3, cell4;
      gsap.delayedCall(0.35, () => {
        cell1 = createCell(0);
        cell3 = createCell(2);
        cell4 = createCell(3);
        
        gsap.set([cell1, cell3, cell4], { left: targets[1].x, top: targets[1].y, scale: 1, opacity: 1 });

        // Slide them to their Row 1 columns
        gsap.to(cell1, { left: targets[0].x, top: targets[0].y, duration: 0.35, ease: "power2.out" });
        gsap.to(cell3, { left: targets[2].x, top: targets[2].y, duration: 0.35, ease: "power2.out" });
        gsap.to(cell4, { left: targets[3].x, top: targets[3].y, duration: 0.35, ease: "power2.out" });
      });

      // ----------------------------------------------------
      // Stage 3: 4 -> 7 Squares (Row 1 splits down to Row 2)
      // ----------------------------------------------------
      let cell5, cell6, cell7;
      gsap.delayedCall(0.85, () => {
        cell5 = createCell(4); // Row 2 Left
        cell6 = createCell(5); // Row 2 Center
        cell7 = createCell(6); // Row 2 Right
        
        gsap.set(cell5, { left: targets[1].x, top: targets[1].y, scale: 1, opacity: 1 });
        gsap.set(cell6, { left: targets[2].x, top: targets[2].y, scale: 1, opacity: 1 });
        gsap.set(cell7, { left: targets[3].x, top: targets[3].y, scale: 1, opacity: 1 });

        // Slide down to Row 2 slots
        gsap.to(cell5, { left: targets[4].x, top: targets[4].y, duration: 0.35, ease: "power2.out" });
        gsap.to(cell6, { left: targets[5].x, top: targets[5].y, duration: 0.35, ease: "power2.out" });
        gsap.to(cell7, { left: targets[6].x, top: targets[6].y, duration: 0.35, ease: "power2.out" });
      });

      // ----------------------------------------------------
      // Stage 4: Immediate Handoff (fade out cells, fade in cards)
      // ----------------------------------------------------
      gsap.delayedCall(1.3, () => {
        const allCells = [cell1, cell2, cell3, cell4, cell5, cell6, cell7];
        allCells.forEach((c, idx) => {
          if (c) {
            gsap.timeline()
              .to(c, { scale: 1.12, opacity: 0, duration: 0.25, ease: "power2.out" })
              .to(targets[idx].card, {
                opacity: 1,
                filter: "blur(0px)",
                duration: 0.35,
                ease: "power2.out",
                onComplete: () => {
                  targets[idx].card.classList.add("animated-active");
                  targets[idx].card.style.pointerEvents = "auto";
                  targets[idx].card.style.opacity = "1";
                  targets[idx].card.style.filter = "none";
                  c.remove();
                }
              }, 0);
          }
        ...
        // Remove overlay after final cells fade out
